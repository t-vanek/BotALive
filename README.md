...
- **Purposeful mining** — tier-gated targets across the **full vanilla ore set** (overworld, deepslate, nether, ancient debris; never mining ore with a tool that drops nothing), access tunnels to buried ore, staircases into the deep (never straight down), vein following, torch placement, and lava/water checks before every block. **Capacity-aware**: a nearly-full backpack stops opening new veins and banks its haul first, so mined drops never despawn onto a full inventory. One central material catalog classifies the whole of vanilla — what's ore, valuable, junk or a building block — so nothing falls through the cracks.
- **Stations & smelting chains** — smoker and blast furnace including prerequisites, a self-sufficient charcoal fuel loop, priority-based fuel selection; worn tools are replaced before they break and repaired at an anvil (crafted and placed if none exists), or **merged at a grindstone** when there's no repair material — two worn unenchanted tools become one working tool, so a bot never gets stuck with broken gear it can't fix. Every workstation is used **by need, not by role**: any bot with the ingredients smelts, smiths, enchants or brews, and self-provisions the station when none is nearby — a profession only shifts *how often*, never *whether*.
...
- **Homes & upkeep** — bots build real houses (walls, door opening, roof, chosen orientation), sleep in beds, bank surplus in chests — not just junk stone but the **mining haul itself** (ores, ingots, coal, gems beyond a working reserve), so a death no longer wipes a day of mining — repair creeper damage against the house plan, and after dying run back for their gear before the drops despawn.
...
- **Finding the End portal** — a bot no longer depends on luck to reach the End: once it has gathered eyes of ender and is geared for the trip, it **locates the nearest stronghold** (server-side structure locate), travels there and scans out the buried **portal frame**, remembering it so the expedition machinery can fill it with eyes and step through. Portals are also learned by walking through one, by exploring, from a friend's gossip, or via `/botalive end portal`.
- **End expeditions** — a bot that knows a portal gears up and fights like an experienced player: shoot the crystals first, never look endermen in the eye, edge-guard against void falls, melee the perched dragon and shoot the flying one. Victory is celebrated in chat, saved as a trophy and raises courage.
...
- **Communal storage** — the granary is a real institution now, not an empty prop: members carry surplus food into it, and a hungry member takes from the settlement's own granary *before* stealing from anyone's chest. Villages also raise a shared **warehouse** that flows **both ways**: members pool surplus materials into it — building blocks *and* the mined ores and ingots beyond each bot's working reserve — and a member short of a material its work needs **draws it back out** instead of going to mine it. A homeless bot pulls building blocks for its first house; a smith or miner out of iron or coal restocks from the shared pile. Miners fill it, others draw — emergent division of labour, biased by profession, fair to the pile (only the deficit, capped per trip).
