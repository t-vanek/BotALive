...
- **Communal storage** — the granary is a real institution now, not an empty prop: members carry surplus food into it, and a hungry member takes from the settlement's own granary *before* stealing from anyone's chest. Villages also raise a shared **warehouse** where members pool surplus materials — building blocks *and* the mined ores and ingots beyond each bot's working reserve — instead of hoarding them in private chests.
