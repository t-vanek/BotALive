...
public enum PhraseCategory {
    ...
    /** Bot začíná stavět radnici sídla ({@code {name}} = jméno sídla). */
    SETTLEMENT_TOWNHALL_START,
    ...
    /** Kostel sídla dokončen ({@code {name}} = jméno sídla). */
    SETTLEMENT_CHURCH_DONE,
    ...
    /** Bot začíná stavět plot kolem svého domu (bez placeholderu). */
    SETTLEMENT_FENCE_START,
    ...
    /** Bot začíná stavět ohradu kolem stáda (bez placeholderu). */
    PEN_START,

    /** Ohrada kolem stáda dokončena (bez placeholderu). */
    PEN_DONE,
