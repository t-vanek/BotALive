...
**Zbývá z tohoto auditu (neopraveno):** dračí trofej jen v celebrate(); EndOuter zombie fáze + perlová smyčka; WitherFight ověření lebek světem; DrinkPotion fire-res mrtvá větev; Guard↔Home noční oscilace; Guard prázdné waypointy; Eat vestoje; CreeperDodge bez EdgeGuard; SellGoal refund zaplacené-zaniklé nabídky; detekce platby deltou zůstatku; TradeGoal mint; Restock↔Supply BOM inflace; Steal sken vlastního sídla; Reconcile given==0; BuildHouse sólo resume + stale latch; SettlementWall isBarrier + PROVISION žere hradbu; BarrierWorker DONE vs. došel materiál; BuildGuard footprint; Supply bezdomovec; MaintainHome hold + blocksRelocation; Mine stop() úklid; Farm PLANT únik; pšeničný řetěz (chleba, kompost rezerva); Craft brána; kapacitní brány Farm/Fish/Hunt; Hunt inProgress; EndTravel brány u portálu; Nether RETURN fallback; Stronghold prefetch; CombatController prahy; utilitySnapshot side-efekty; forced-goal zmrazení; settlement-roads sirotek + RoleCoverageTest zrcadlo; NETHERITE poslední míle; DimensionPolicy NETHER tabulka; walls pause politika; resumption decay vs. noc.
